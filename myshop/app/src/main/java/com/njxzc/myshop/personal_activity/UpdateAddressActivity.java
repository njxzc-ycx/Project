...
public class UpdateAddressActivity extends AppCompatActivity {
    ...
    private void init() {
        ...
        Model.getInstance().getGlobalThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                String url =  OkhttpHelper.getIp()+"/shopsystem/androidAddress/findAddressByAddressId";
                OkhttpHelper.findAddressByAddressId(url, String.valueOf(addressId), new Callback() {
                    @Override
                    public void onFailure(Call call, IOException e) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(UpdateAddressActivity.this, "网络连接失败", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        try {
                            ...
                            final int defaultAddress = address.getInt("defaultAddress");
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    et_linkman.setText(linkman);
                                    et_linkphone.setText(linkphone);
                                    tv_area.setText(area);
                                    et_addressDetail.setText(addressDetail);
                                    if (defaultAddress==1){
    ...
    public void submit(View v) throws Exception{
        ...
        if (linkman==""||linkman.length()==0||linkman==null) {
            Toast.makeText(UpdateAddressActivity.this,"收件人不得为空",Toast.LENGTH_SHORT).show();
        }else if (addressDetail==""||addressDetail.length()==0||addressDetail==null){
            Toast.makeText(UpdateAddressActivity.this,"详细地址不得为空",Toast.LENGTH_SHORT).show();
        } else if(!judgePhoneNums(linkphone)){
            return;
        }else {
            //start the progress dialog
            mProgressDialog = ProgressDialog.show(UpdateAddressActivity.this, "", "正在提交...");
            Model.getInstance().getGlobalThreadPool().execute(new Runnable() {
                @Override
                public void run() {
                    try {
                    ...
                    String url = OkhttpHelper.getIp() + "/shopsystem/androidAddress/updateAddress";
                    OkhttpHelper.updateAddress(url, String.valueOf(userId),String.valueOf(addressId),addressDetail, area, linkphone, linkman, String.valueOf(isdefault), new Callback() {
                        @Override
                        public void onFailure(Call call, IOException e) {
                            runOnUiThread(new Runnable() {
                                @Override
                        ...
                        public void onResponse(Call call, Response response) throws IOException {
                            ...
                            if (responseData.equals("updateSuccess")) {
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        Toast.makeText(UpdateAddressActivity.this, "修改地址成功", Toast.LENGTH_SHORT).show();
                                        Intent intent = new Intent("android.intent.action.CART_BROADCAST");
                                        intent.putExtra("flag", "refreshAddress");
                                        LocalBroadcastManager.getInstance(UpdateAddressActivity.this).sendBroadcast(intent);
                                ...
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        Toast.makeText(UpdateAddressActivity.this, "修改地址失败", Toast.LENGTH_SHORT).show();
                                    }
                                });
