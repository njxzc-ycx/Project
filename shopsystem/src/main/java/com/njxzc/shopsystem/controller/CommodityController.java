...
import com.njxzc.shopsystem.pojo.Search;
...
public class CommodityController {
    ...
    public String allCommoditys()throws Exception{
        return "allCommoditys";
    }

    @RequestMapping("/allAuditCommoditys")
    public String allAuditCommoditys()throws Exception{
        return "allAuditCommoditys";
    }

    @RequestMapping(value ="/findAllCommodity")
    @ResponseBody
    public Map<String,Object> findAllCommodity(@RequestParam(required=false,defaultValue="1") int page,
                                               @RequestParam(required=false) int limit) {
        // 使用Pagehelper传入当前页数和页面显示条数会自动为我们的select语句加上limit查询
        // 从他的下一条sql开始分页
        PageHelper.startPage(page, limit);
        List<Commodity> allCommoditys  = commodityService.findAllCommoditys();
        System.out.println(allCommoditys);
        // 使用pageInfo包装查询
        PageInfo<Commodity> rolePageInfo = new PageInfo<>(allCommoditys);//*/
    ...
    @RequestMapping(value ="/findallAuditCommoditys")
    @ResponseBody
    public Map<String,Object> findallAuditCommoditys(@RequestParam(required=false,defaultValue="1") int page,
                                               @RequestParam(required=false) int limit) {
        // 使用Pagehelper传入当前页数和页面显示条数会自动为我们的select语句加上limit查询
        // 从他的下一条sql开始分页
        PageHelper.startPage(page, limit);
        List<Commodity> allAuditCommoditys  = commodityService.findallAuditCommoditys();
        System.out.println(allAuditCommoditys);
        // 使用pageInfo包装查询
        PageInfo<Commodity> rolePageInfo = new PageInfo<>(allAuditCommoditys);//*/
        Map<String,Object> map=new HashMap<String,Object>();
        map.put("code",0);
        map.put("msg","");
        map.put("count",rolePageInfo.getTotal());
    ...
    @RequestMapping(value="/adminFindCom",produces="application/json;charset=utf-8")
    @ResponseBody
    public Map<String, Object> adminFindCom(Integer page, Integer limit,String start,String end,String key,int flag) {
        // 使用Pagehelper传入当前页数和页面显示条数会自动为我们的select语句加上limit查询
        // 从他的下一条sql开始分页
        ...
        PageHelper.startPage(page, limit);
        List<Commodity> allCommoditys  = commodityService.adminFindCom(start, end, key,flag);
        System.out.println(allCommoditys);
        // 使用pageInfo包装查询
        PageInfo<Commodity> rolePageInfo = new PageInfo<>(allCommoditys);//*/
        Map<String,Object> map=new HashMap<String,Object>();
    ...
    @GetMapping("/toBuyComDetail/{comId}")
    public ModelAndView toBuyComDetail(ModelAndView mav,@PathVariable("comId")int comId)throws Exception{
        //1.查询商品信息
        Commodity commodity = commodityService.findCommodityByComId(comId);
        //2.解析其他多个图片
        List<String> urlList = commodityService.dealComImageOther(commodity.getComImageOther());
    ...
    @GetMapping("/updateComFlagByComId")
    @ResponseBody
    public void updateComFlagByComId(int comId,int flag)throws Exception{
        Commodity commodity = new Commodity();
        commodity.setComId(comId);//将得到的编号放入Commodity对象
        commodity.setFlag(flag);//将得到的原始状态放入Commodity对象
        System.out.println("Id与原始flag"+commodity.toString());
        //1.调用修改方法
        commodityService.updateComFlagByComId(commodity);
    }
